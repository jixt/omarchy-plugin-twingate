...
Runs unsandboxed with your user permissions, same as any other Omarchy shell plugin. It only ever runs `twingate` and the `omarchy-launch-browser` helper listed above — no elevated privileges are requested or required, and it does not install or update packages.
